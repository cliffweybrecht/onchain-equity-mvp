...
async function main() {
  ...
  const abi = parseAbi([
    ...
    "function emergencyFreeze(string reason)",
  ]);

  console.log("\n== OPS EMERGENCY FREEZE ==");
  ...
  if (before === true) {
    console.log("✅ Already frozen. No action taken.");
    return;
  }

  const reason =
    process.env.REASON ||
    `pilot-break-glass freeze (Part 3.8.2) @ ${new Date().toISOString()}`;
  ...
  const after = await pollFrozen(publicClient, emergency, abi, true, 12);
